...
export default function GeospatialThreatMap({ heatmaps = [], selectedRegion = '', onSelectRegion }) {
  ...
  const getThreatColor = (level) => {
    switch (level) {
      ...
      case 'Medium Risk':
        return '#F59E0B'; // Amber
      case 'Monitored':
        return '#1769AA'; // Primary Brand Blue
      case 'Low / Safe':
      default:
  ...
  const getThreatBadgeClass = (level) => {
    switch (level) {
      ...
      case 'Monitored':
        return 'bg-blue-50 text-[#1769AA] border border-blue-200';
      case 'Low / Safe':
      default:
  ...
  return (
    <div className="p-5 rounded-2xl bg-white border border-[#E2E8F0] flex flex-col justify-between shadow-sm shadow-blue-900/5 space-y-4">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-100 pb-3">
        <div>
          <h3 className="font-bold text-base text-[#12355B] flex items-center gap-2 font-sans">
            <Globe2 className="w-5 h-5 text-[#1769AA]" />
        ...
        <div className="flex items-center gap-2">
          {selectedRegion ? (
            <div className="flex items-center gap-1.5 text-xs font-mono px-3 py-1 rounded-full bg-blue-50 text-[#1769AA] border border-blue-200 font-semibold shadow-sm">
      ...
      {/* Map Legend */}
      <div className="flex flex-wrap items-center gap-4 text-xs font-sans px-3 py-2 rounded-xl bg-slate-50 border border-slate-200">
        <span className="text-[#4A607A] font-medium">Threat Level Scale:</span>
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-[#EF4444] shadow-sm"></span>
          <span className="text-rose-700 font-mono font-semibold">High Danger</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-[#F59E0B] shadow-sm"></span>
          <span className="text-amber-800 font-mono font-semibold">Medium Risk</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-[#1769AA] shadow-sm"></span>
          <span className="text-[#1769AA] font-mono font-semibold">Monitored</span>
        </div>
        <div className="flex items-center gap-1.5">
          <span className="w-2.5 h-2.5 rounded-full bg-[#10B981] shadow-sm"></span>
          <span className="text-emerald-700 font-mono font-semibold">Low / Safe</span>
        </div>
      </div>

      {/* Leaflet Light Map Container */}
      <div className="relative rounded-xl overflow-hidden border border-slate-200 h-[440px] z-10 shadow-inner">
        <MapContainer
                    ...
                    <div className="p-3.5 space-y-2.5 font-sans min-w-[220px]">
                      <div className="flex items-center justify-between border-b border-slate-100 pb-2">
                        <div>
                          ...
                          <div className="text-[11px] text-[#4A607A] font-mono">{node.state}, India</div>
                        </div>
                      ...
                      <div className="space-y-1.5 text-xs">
                        <div className="flex justify-between text-[#4A607A]">
                          <span>Active Alerts:</span>
                          <span className="font-bold font-mono text-[#12355B]">{node.threats} Threats ({node.posts} posts)</span>
                        </div>
                        <div className="flex justify-between text-[#4A607A]">
                      ...
                      <button
                        ...
                        className="w-full mt-2 py-1.5 rounded-lg bg-[#1769AA] hover:bg-[#13558A] text-white text-xs font-semibold transition flex items-center justify-center gap-1.5 shadow-sm"
                      >
      ...
      <div className="space-y-2">
        <div className="flex items-center justify-between text-xs font-mono text-[#4A607A]">
          ...
          {selectedRegion && (
            <button 
              onClick={handleClearSelection}
              className="text-[#1769AA] hover:underline flex items-center gap-1 text-[11px] font-semibold"
            >
            ...
            return (
              <div
                ...
                className={`p-2.5 rounded-xl border transition cursor-pointer flex items-center justify-between ${
                  isSelected
                    ? 'bg-blue-50/90 border-[#1769AA] text-[#12355B] shadow-sm'
                    : 'bg-slate-50 border-slate-200 hover:border-slate-300 text-[#12355B]'
                }`}
              >
                <div className="flex items-center gap-2">
                  <MapPin className={`w-3.5 h-3.5 ${
                    node.level === 'High Danger' ? 'text-rose-600' :
                    node.level === 'Medium Risk' ? 'text-amber-600' :
                    node.level === 'Monitored' ? 'text-[#1769AA]' : 'text-emerald-600'
                  }`} />
                  <div>
                    <div className="font-bold text-[#12355B] text-xs">{node.name}</div>
                    <div className="text-[10px] text-[#4A607A] truncate max-w-[130px]">
                      {node.topic}
                    </div>
                  </div>
                </div>
                ...
                <div className="text-right">
                  ...
                  <span className="text-[10px] text-[#4A607A] block mt-0.5">{node.posts} posts</span>
                </div>
              </div>
            );
          })}
