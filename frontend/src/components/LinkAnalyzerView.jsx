    ...
    <div className="space-y-6">
      ...
      <div className="p-6 rounded-2xl bg-[#081836] border border-blue-900/80 shadow-xl space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-blue-900/80 pb-4">
          ...
          <div className="flex items-center gap-2 font-sans">
            <div className="px-3 py-1.5 rounded-xl bg-[#040d21] border border-blue-900/80 text-xs">
              <span className="text-blue-300">Links Checked: </span>
            ...
            <div className="px-3 py-1.5 rounded-xl bg-blue-950 border border-blue-700 text-xs">
              <span className="text-blue-200">Phishing Scams: </span>
              <span className="text-white font-bold font-mono">{linkStats?.phishing_count || 0}</span>
            </div>
            <div className="px-3 py-1.5 rounded-xl bg-blue-950 border border-blue-700 text-xs">
              <span className="text-blue-200">Dangerous Files: </span>
        ...
        <form onSubmit={(e) => { e.preventDefault(); handleScan(); }} className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input
              ...
              className="w-full pl-10 pr-4 py-2.5 rounded-xl bg-[#040d21] border border-blue-900/80 focus:border-blue-400 outline-none text-sm text-white font-mono placeholder:text-blue-400/50 transition"
            />
          </div>
          <button
            type="submit"
        ...
        <div className="flex flex-wrap items-center gap-2 pt-1 text-xs font-sans">
          <span className="text-blue-300 font-medium">Try Sample Links:</span>
          <button
            onClick={() => {
            ...
            className="px-2.5 py-1 rounded-lg bg-[#040d21] border border-blue-900 hover:border-blue-400 text-blue-200 transition"
          >
            Fake Bank KYC Link
          </button>
      ...
      {scanResult && (
        <div className="p-6 rounded-2xl bg-[#081836] border border-blue-900/80 shadow-xl space-y-5 font-sans">
              ...
              <div className={`p-4 rounded-xl ${badge.bg} border ${badge.border} flex flex-col sm:flex-row sm:items-center justify-between gap-3`}>
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-xs text-blue-200 truncate max-w-lg mt-0.5 font-mono">
                      {scanResult.url}
                    </p>
                  </div>
                </div>
                ...
                <div className="text-right">
                  ...
                  <span className="text-[10px] text-blue-300">Safety Check Rating</span>
                </div>
              </div>
            );
          })()}
          ...
          <div className="space-y-2.5">
            ...
            <div className="p-4 rounded-xl bg-[#040d21] border border-blue-900/80 space-y-3 font-sans">
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                {getRedirectionChain(scanResult).map((hop) => (
                  <div
                    key={hop.hop}
                    className="p-3 rounded-lg bg-[#081836] border border-blue-900/80 text-xs space-y-1 relative"
                  >
                    <div className="flex items-center justify-between text-[11px]">
                      ...
                      <span className={`px-1.5 py-0.5 rounded font-bold font-mono ${
                        hop.status === 200 ? 'bg-blue-950 text-white' : 'bg-blue-950 text-blue-300'
                      }`}>
                        HTTP {hop.status}
                      </span>
                    </div>
                    <div className="font-bold text-white truncate font-mono">{hop.domain}</div>
                    <div className="text-[10px] text-blue-300 truncate font-mono">{hop.url}</div>
                    <div className="flex justify-between items-center text-[10px] pt-1 text-blue-400">
                      <span>Status:</span>
                      <span className="text-blue-200 font-medium">{hop.risk}</span>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4 text-xs">
            <div className="p-4 rounded-xl bg-[#040d21] border border-blue-900/80 space-y-2">
              <span className="font-bold text-white flex items-center gap-1.5">
                <AlertTriangle className="w-4 h-4 text-blue-400" /> Why is this link dangerous?
              </span>
            ...
            <div className="p-4 rounded-xl bg-[#040d21] border border-blue-900/80 space-y-2">
              <span className="font-bold text-white flex items-center gap-1.5">
                <Globe className="w-4 h-4 text-blue-400" /> Website Host Information:
              </span>
              <div className="space-y-1 text-blue-200 font-mono">
      ...
      <div className="p-6 rounded-2xl bg-[#081836] border border-blue-900/80 shadow-xl space-y-4">
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-left text-xs font-sans">
            <thead>
              <tr className="border-b border-blue-900/80 text-blue-300 text-[11px]">
            ...
            <tbody className="divide-y divide-blue-900/80">
              {links.map((item) => (
                <tr
                  key={item.id}
                ...
                >
                  <td className="py-3.5 pr-4 max-w-sm">
                    <div className="font-bold text-white group-hover:text-blue-200 truncate font-mono">
                      {item.url}
                    </div>
                  ...
                  <td className="py-3.5 pr-4">
                    <span className={`px-2 py-0.5 rounded font-bold text-[11px] ${
                      item.threat_type === 'Clean' ? 'bg-blue-950 text-white border border-blue-600' :
                      item.threat_type === 'Phishing' ? 'bg-blue-900 text-white border border-blue-500' :
                  ...
                  <td className="py-3.5 pr-4 font-bold text-blue-100 font-mono">
                    {item.confidence_score}%
                  </td>

                  <td className="py-3.5 pr-4 max-w-xs">
                    <p className="text-blue-200 text-xs truncate font-sans">
                      {item.risk_factors}
                    </p>
                  </td>
                  ...
                  <td className="py-3.5 text-right">
                    <button
                      ...
                      className="px-3 py-1.5 rounded-lg bg-[#040d21] hover:bg-blue-950 border border-blue-800 hover:border-blue-500 text-blue-200 text-[11px] transition ml-auto font-sans font-medium"
                    >
                      View Details
                    </button>
                  </td>
